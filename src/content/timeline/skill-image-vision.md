...
- 三步流程：定位图片来源 → 格式准备 → 调 `mcp__MiniMax__understand_image`
- 图片来源优先级：用户给的路径/URL → 粘贴附件（先跑 `clipimg`，失败则找临时目录最近的 `image*.png`）→ glob 查找
- 格式约束：`understand_image` 只支持 JPEG/PNG/WebP，其他格式用 `sips` 转换
...
- 视觉工具：MiniMax `understand_image`（MCP，支持本地文件路径与 HTTP URL）

## 验证

粘贴截图实测通过：`clipimg` 正确落盘 948×556 PNG，`understand_image` 完整描述了截图中的界面与文字内容。
