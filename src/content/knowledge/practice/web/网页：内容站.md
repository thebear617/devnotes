...
1. 侧边目录栏要注意做sticky，不然页面向下滚动后，会不被带着走
2. 本地服务的路由端口最好固定下来
3. sol 前端能力一般，可以先让 codex 生成视觉效果图再移植开发，或者先写 prompt 去 AIStudio 的 build 生成 apps，然后下载源码丢给 codex，其次是让 grok4.6 先生成前端 ui mvp 再丢给 codex 后续开发也行，以及还能选 Sonnet5 性价比高 效果比 GPT 强很多很多
...
```markdown
> 自己的通用逻辑 ｜26-09-10 追加｜26-09-10 更新
...
3. 接着考虑每个容器/组件的内部布局 
  1. 逻辑：下滑、悬停：气泡 点击：下拉菜单、悬浮窗、抽屉、展开 
