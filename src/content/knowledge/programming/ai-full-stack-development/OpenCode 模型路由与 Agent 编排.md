...
| `@route-build` | `opencode-go/deepseek-v4-flash` | 实际修改代码、内容和配置，运行必要验证 | 编辑和 Bash 需要确认 |
| `@route-review` | `opencode-go/deepseek-v4-pro` | 检查 diff、测试、构建、发布风险和敏感信息 | 禁止编辑，Bash 需要确认 |
| `@route-docs` | `opencode-go/qwen3.7-plus` | 中文 Markdown、文档、注释和结构化内容 | 编辑和 Bash 需要确认 |

默认模型是 `opencode-go/deepseek-v4-flash`。如果任务只是搜索、解释或实际改代码，优先使用 Flash；复杂规划使用 GLM 5.2；发布前审查才使用 DeepSeek V4 Pro；中文文档使用 Qwen3.7 Plus。
...
- 6 个 Agent 文件的 frontmatter、模型映射和权限均已检查；
