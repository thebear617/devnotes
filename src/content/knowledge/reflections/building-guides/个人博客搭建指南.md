...
**问题描述：**本地 Astro 开发服务器中，从英雄联盟页面点击「路由表」后，根路径会被环境规则重定向为 `/notes/`，而熊窝没有该路由，因此显示 404 Not Found。

**解决方案：**开发环境将「路由表」入口明确指向 `/index.html`；该静态入口返回 200，避开根路径重定向。线上构建仍使用熊窝主入口，不受本地规则影响。
