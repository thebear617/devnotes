export const notes = [
  {
    id: 'pig-home-food-map-region-pagination-css-debug',
    product: '猪窝',
    stacks: ['功能'],
    langs: ['CSS'],
    type: '内容',
    title: '猪窝美食地图区域列表分页：CSS 优先级 Debug',
    date: '2026-08-15',
    body: `<p>记录区域列表分页显示异常的排查与修复过程。</p>
...
<ul>
  ...
  {
    id: 'lifenotes-blog-width-debug-article-detail',
    product: 'LifeNotes',
    stacks: ['前端', 'CSS', '布局', '调试'],
    langs: ['CSS', 'Astro'],
    type: '调试',
    title: 'LifeNotes 博客详情页宽度：修复内容少的文章按内容收缩',
    date: '2026-08-14',
    body: `<h2>① 现象</h2>
...
<ul>
  ...
  {
    id: 'chatnotes-two-column-scroll-ownership',
    product: 'ChatNotes',
    stacks: ['前端', 'CSS', '布局', '调试'],
    langs: ['TypeScript', 'CSS'],
    type: '调试',
    title: 'ChatNotes 双栏滚动：让左侧固定、右侧独立滚动',
    date: '2026-08-12',
    body: `<h2>① 目标交互</h2>
...
<p><strong>维护原则：</strong>以后调整双栏布局时，优先检查高度约束和滚动容器归属；不要只给某个元素追加 sticky，也不要把底部输入框重新改成 fixed，除非同时为内容区预留等高的安全空间。</p>`,
  ...
  {
    id: 'devnotes-astro-node-modules-ssr-timeout',
    product: 'Astro / Vite',
    stacks: ['前端', '开发工具', '调试', '依赖管理'],
    langs: ['JavaScript', 'Shell', 'Markdown'],
    type: '调试',
    title: 'DevNotes 启动卡在 types？Astro 内容模块超时与 node_modules 重装恢复',
    date: '2026-08-09',
    body: `<h2>① 现象</h2>
...
<p><strong>实践建议：</strong>生产环境保留域名白名单；本地开发使用单独的开发 Key，或在仅用于本地调试的 Key 上不设置白名单，不要为了本地调试而放开生产 Key 的限制。</p>`,
  ...
  },
  {
    ...
    langs: ['Python', 'Shell'],
    type: '调试',
    title: 'bili-audio-transcribe skill 修复实录：B 站转录卡下载的 6 步根因拆解',
    date: '2026-08-05',
    body: `<h2>① 现象</h2>
...
<ol>
  ...
  <li>排障先分清「skill 自身 bug / 外部风控 / 环境噪声」三类原因，再看文档是否与实现一致</li>
    ...
    links: [
      { title: 'yt-dlp 官方仓库', url: 'https://github.com/yt-dlp/yt-dlp' }
    ]
  },
  {
    id: 'chrome-localhost-stale-cache-notes-redirect',
    product: 'Chrome / Astro',
    stacks: ['前端', '开发工具', '调试'],
    langs: ['JavaScript'],
    type: '调试',
    title: 'localhost:4321 明明是当前站点，Chrome 却跳到 /notes/？先查旧缓存与端口复用',
    date: '2026-08-02',
    body: `<h2>① 现象</h2>
...
<p><strong>排查口诀：</strong>先用 <code>curl</code> 判断服务端有没有重定向，再用不同浏览器对比；只有 Chrome 异常时，优先检查缓存和端口是否被其他项目复用。</p>`
  ...
  {
    id: 'astro-public-images-base-url',
    product: 'Astro',
    stacks: ['前端', '静态站点', '部署'],
    langs: ['JavaScript'],
    type: '调试',
    title: 'Astro 迁移后图片全碎？同时检查 public 子路径与 BASE_URL 尾部斜杠',
    date: '2026-07-29',
    body: `<p><strong>症状：</strong>旧的静态站迁到 Astro 后，猫咪卡片仍正常渲染，但所有 <code>&lt;img&gt;</code> 都显示破图。图片已经放在 <code>public/images/</code>，构建产物中也存在对应文件。</p>
...
<ol>
  ...
  <li>检查 <code>dist/images/</code> 是否包含图片，再运行 <code>npm run build</code>。</li>
    ...
    links: [
      { title: 'Astro 文档：base 配置', url: 'https://docs.astro.build/en/reference/configuration-reference/#base' },
      { title: 'Astro 文档：public 目录', url: 'https://docs.astro.build/en/basics/project-structure/#public' }
    ]
  },
  {
...
<p>排查 skill 加载问题记住口诀：<strong>报错不可信 → 注册表可疑 → 进程年龄可疑 → 重启解决一切</strong>。</p>`,
  ...
  },
  {
    ...
    langs: ['Shell'],
    type: '调试',
    title: 'MiniMax API Key 更换：四个位置 + 两个重启',
    date: '2026-08-02',
    body: `<h2>适用场景</h2>
