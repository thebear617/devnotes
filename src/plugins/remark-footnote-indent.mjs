...
function splitLazyNestedLists(list) {
  // A second-level list indented only two spaces beyond its parent is parsed as
  // lazy continuation lines of the parent item's paragraph. Split those marker
  // lines back out into a nested ordered list so both two- and four-space
  // nesting render the same inside footnotes.
